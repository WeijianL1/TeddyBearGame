...
Adventure.handleCommand = function(line) {
   ...
   if(Adventure.cmdList.indexOf(cmd) == -1){
      ...
      responsiveVoice.speak("I don't understand that word.", "UK English Female", 
          {pitch: 1},{rate:0.5});}
   else {
...
Adventure.handleTalkCommand = function(cmd, rest){
    if(cmd == "TALK"){
        ...
        if(count(people) == 0){
            ...
            responsiveVoice.speak("There is no one else in the room.", "UK English Female", 
          {pitch: 1},{rate:0.5});
        }
        for(var p in people){
            println(p);
            responsiveVoice.speak(p, "UK English Female", 
          {pitch: 1},{rate:0.5});
        }
    }
}
...
Adventure.handleObjectCommand = function(cmd, item) {
   if(cmd == "TAKE") {
      if(item in Adventure.currentRoom.objects) {
         ...
         println(item + " taken.");
         responsiveVoice.speak(item + " taken.", "UK English Female", 
          {pitch: 1},{rate:0.5});
      } else {
         println("I don't see " + item + " here.");
         responsiveVoice.speak("I don't see " + item + " here.", "UK English Female", 
          {pitch: 1},{rate:0.5});
      }
   } else if(cmd == "DROP") {
      if(item in Adventure.inventory) {
         ...
         responsiveVoice.speak(item + " dropped.", "UK English Female", 
          {pitch: 1},{rate:0.5});
      } else {
         println("You do not have " + item + " in your inventory.");
         responsiveVoice.speak("You do not have " + item + " in your inventory.", "UK English Female", 
          {pitch: 1},{rate:0.5});
      }
   }
}